...
static void invertOpenMP(const Image& input, Image& output) {//инверсия цветов
    ensureOutputLike(input, output);
    const int n = input.bytes();
    #pragma omp parallel for schedule(static)//дележка между потоками
    for (int i = 0; i < n; ++i) {
        output.data[i] = static_cast<Byte>(255 - input.data[i]);
    }
}

static void medianOpenMP(const Image& input, Image& output) {//медиана (не берем крайние пиксели птмчт нет соседей 3х3 не получится)
    ensureOutputLike(input, output);
...
static void sobelOpenMP(const Image& input, Image& output) {
    ensureOutputLike(input, output);
    ...
    #pragma omp parallel for schedule(static)//параллелим
...
static void printUsage() {//инструкция 
    cout
        << "Usage:\n"
        << "  openmp.exe [input_file_or_dir] [output_dir] [repeats]\n\n"
        << "Defaults:\n"
...
static void applyFilter(Filter filter,  const Image& input, Image& output) {//выбор нужного фильтра
    if (filter == Filter::Invert) invertOpenMP(input, output);
    if (filter == Filter::Median) medianOpenMP(input, output);
    if (filter == Filter::Sobel)  sobelOpenMP(input, output);
}
        ...
        for (const fs::path& file : inputs) {
            ...
            for (Filter filter : filters) {
                ...
                fs::path outPath = outputDir /
                    (file.stem().string() + "_" + filterName(filter) + "_openmp" + ".png");
